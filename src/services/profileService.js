...
export const connectWithExtra1 = async function (proxy) {
  const { name, url, username, password, profileName, userAgent } = proxy;
  const options = {
    headless: false,
    args: [
      "--disable-webgl",
  ...
  puppeteer
    ...
    .then(async (browser) => {
      ...
      // const searchResultLinks = await page.$$eval('div.g > a', links =>
      // links.map(link => link.href));
      // const urlPattern = /https?:\/\/vnexpress\.net\/\d+/;

      // console.log(searchResultLinks)
      // // Find the first search result link that matches the predefined pattern
      // const matchingLink = searchResultLinks.find(link =>
      //   urlPattern.test(link)
      // );

      // if (matchingLink) {
      //   // Click on the matching link
      //   await Promise.all([
      //     page.waitForNavigation(),
      //     page.goto(matchingLink),
      //   ]);
    
      //   console.log(`Clicked on matching link: ${matchingLink}`);
      // } else {
      //   console.log('No matching link found.');
      // }

      let searchResultLinks = await page.evaluate(() => {
        let results = [];
    ...
    console.log(searchResultLinks)
    const urlPattern = /https?:\/\/dantri\.com\.vn\/\d+/;
      ...
      console.log(`Search results for "${searchQuery}" loaded successfully!`);
