...
import { connect, connectWithExtra, connectWithExtra1 } from "./profileService";
...
export async function ipcMainHandle() {
  ...
  ipcMain.handle("connectWithExtra1", async (event, proxy) => {
    const result = await connectWithExtra1(proxy);
    return result;
  });
